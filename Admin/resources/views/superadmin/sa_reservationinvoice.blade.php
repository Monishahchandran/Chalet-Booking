        ...
        <div class="x_content">
            ...
            <div class="col-md-6 col-sm-12 col-xs-6">
                ...
                <address>
                    ...
                    Total Reservations :
                    @if(empty($chaletdetails))
                    <a class="btn btn-info  btn-xs" href="{{ url('/Total-Reservations') }}/<?php echo $id; ?>">{{(new \App\Helper)->get_count_chaletreservation($ownerdetails->id) }}</a>
                    @else
                    <?php $cid = base64_encode($chaletdetails->id); ?>
                    <a class="btn btn-info  btn-xs" href="{{ url('/TotalReservations') }}/<?php echo $cid; ?>/<?php echo $id; ?>">{{(new \App\Helper)->get_count_ownerchaletreservation($chaletdetails->id) }}</a>
                    @endif
                    ...
                    <br>Total Amount : <strong>
                        @if(empty($chaletdetails))
                        {{(new \App\Helper)->get_owner_totalpaid($ownerdetails->id) }}
                        @else
                        {{(new \App\Helper)->get_chalet_totalpaid($chaletdetails->id) }}
                        @endif
                    </strong> KD
                    <br>Total Commission : <strong>
                        @if(empty($chaletdetails))
                        {{(new \App\Helper)->get_owner_commission($ownerdetails->id) }}
                        @else
                        {{(new \App\Helper)->get_chalet_commission($chaletdetails->id) }}
                        @endif
                    </strong> KD
                    <br>Total Amount after Commission : <strong>
                        ...
                        @if((new \App\Helper)->get_chalet_totalpaid($ownerdetails->id) >(new \App\Helper)->get_chalet_commission($ownerdetails->id))
                        {{(new \App\Helper)->get_chalet_totalpaid($ownerdetails->id)-(new \App\Helper)->get_chalet_commission($ownerdetails->id)}}
                        @else{
                        {{(new \App\Helper)->get_chalet_commission($ownerdetails->id)-(new \App\Helper)->get_chalet_totalpaid($ownerdetails->id)}}
                        }
                        @endif
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <?php $id = base64_encode($rdetails->cid);
                                $page = base64_encode('totalreservation'); ?>
