                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td align="center">
                            ...
                            <label><input type="checkbox" id="booking" class="js-switch" name="booking_status" onclick="mybookingFunction('<?php echo $cdetails->cid; ?>')"></label>
                            @endif
                            @if($cdetails->auto_acceptbooking==1)
                            <label><input type="checkbox" name="booking_status" id="booking" class="js-switch" checked onclick="mybookingFunction('<?php echo $cdetails->cid; ?>')"></label>
                            @endif
                        </td>
                        <td align="center">
                            @if($cdetails->is_activestatus==1)
                            <label><input type="checkbox" id="myCheck" onclick="myFunction('<?php echo $cdetails->cid; ?>')" class="js-switch" name="chalet_status" checked> </label>
                            @endif
                            @if($cdetails->is_activestatus==0)
                            <label><input type="checkbox" id="myCheck" onclick="myFunction('<?php echo $cdetails->cid; ?>')" class="js-switch" name="chalet_status"> </label>
                            @endif
                        </td>
                        <td align="center">
                            <?php $id = base64_encode($cdetails->cid); ?>
                            <a class="btn btn-primary btn-xs" href="{{ url('/Chalet-edit') }}/<?php echo $id; ?>" >Edit</a>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
<script type="text/javascript">
    ...
    function mybookingFunction($cid) {
        // alert($cid)
        var checkBox = document.getElementById("booking");
        if (checkBox.checked == true) {
            // alert("checked");
            var check = 1;
        } else {
